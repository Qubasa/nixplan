...
- [x] 1.2 This change's two delta specs are already registered in `excused` in
...
- [x] 1.3 **Leave every checkbox of this file unchecked until the landing edit of task 8.3.**
...
- [x] 1.5 Record the entry point inventory this change changes the signature of, so phase 2 edits
...
- [x] 2.1 In `image/read.nix`, add `valueIndex`, one index over a plan keyed by the path each
...
- [x] 2.2 In `image/read.nix`, add `valuesOf { valueIndex, key, entry }` answering
...
- [x] 2.3 In `image/read.nix`, make `hostPathsOf` take `generated` instead of calling
...
- [x] 2.4 In `image/read.nix`, make `referencePaths` (`:809-814`) and the attachment description's
...
- [x] 2.5 In `image/read.nix`, change `hostPaths`, `denials` and `versionFor` to take `generated`
...
- [x] 2.6 In `operator/read.nix`, build the index once per reading between the `{ plan, realise }`
...
- [x] 2.7 Confirm `image/default.nix` needs no edit: the mount points map `image.hostPaths`
...
- [x] 3.1 In `operator/read.nix`, produce `operator-entry-value-unaccounted`: one error row per
...
- [x] 3.2 In `image/read.nix`, add the account carrying that identifier to the `accounts` table
...
- [x] 3.3 Register `operator-entry-value-unaccounted` in the row tables of `docs/diagnostics.md`,
...
- [x] 4.1 Verify the version digest needs no edit and moves anyway: `versionFor` takes the digest
...
- [x] 4.2 Verify the golden plan is byte-identical: this change adds no plan field and no key input
...
- [x] 5.1 In `tests/unit/image.nix`, add the scenarios of the `realiser/portable-service-image`
...
- [x] 5.2 In `tests/unit/image.nix`, add the denial scenarios:
...
- [x] 5.3 In `tests/unit/operator.nix`, verify the mirrored reading with no edit in it beyond 2.6
...
- [x] 6.1 In `tests/e2e/portable-image/deployment/interfaces/default.nix` and
...
- [x] 6.2 In `tests/e2e/portable-image/deployment/`, add one consumer module and one instance:
...
- [x] 6.3 In `tests/e2e/portable-image/test_portable_image.py`, add the phase and its three cases:
...
- [x] 6.4 Verify the flakelet half needs no folder and no edit: `tests/e2e/shared-postgres/` already
...
- [x] 7.1 In `openspec/changes/deliver-a-secret-without-exposing-it/tasks.md`, mark tasks 6.1
...
- [x] 8.1 In `docs/operator.md`, state which values an entry is shown - the ones its own declaration
...
- [x] 8.2 Run the gates: `nix eval --json '.#debug.failures'` returns `[]`;
...
- [x] 8.3 In **one** edit, now that every scenario has its test: delete this change's two `excused`
