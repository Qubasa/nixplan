...
- [x] 1.1 Record no perf baseline, and record why: this change edits nothing `perf/eval.nix`
  ...
  budget.
- [x] 1.2 Verify that this change's five delta specs are already registered in `excused` in
...
- [x] 1.3 **Leave every checkbox of this file unchecked for as long as those excuses stand.**
...
- [x] 2.1 Create the top-level `modules/` directory with the two modules' homes -
...
- [x] 2.2 Add `modules/flake-module.nix` and name it in the `imports` of `flake.nix:43-48`, the way
...
- [x] 2.3 Register a new unit suite `tests/unit/modules.nix` in `suites` in `tests/default.nix` -
...
- [x] 2.4 Verify no other registration point is tripped: `cli/enrollment.py` is inside a directory
...
- [x] 3.1 In `operator/read.nix`, take one more statement beside `realise` in the argument set at
...
- [x] 3.2 In `operator/read.nix`, publish the coordination facts into the deployment record beside
...
- [x] 3.3 In `cli/manifest.py`, decode the coordination record and refuse a malformed one where the
...
- [x] 3.4 Register the new row identifier in the row tables of `docs/diagnostics.md` and confirm
...
- [x] 4.1 Write `modules/coordination/` as a leaf module whose function arguments are the facts it
  ...
  error row.
- [x] 4.2 In `modules/coordination/`, make every other cluster-only choice a setting whose default
...
- [x] 4.3 In `modules/coordination/`, render both objects from one derivation of the facts the
...
- [x] 4.4 In `modules/coordination/`, declare the credential generator and its program so that the
...
- [x] 5.1 Add `cli/enrollment.py`: read the coordination entry off the deployment record, refuse
...
- [x] 5.2 In `cli/remote.py`, add the one step a verb takes, addressed by the machine's scope in
...
- [x] 5.3 In `cli/enrollment.py`, implement the minting verb: run the credential value's own
...
- [x] 5.4 In `cli/enrollment.py`, implement the listing and the expulsion: the listing prints what
...
- [x] 5.5 In `cli/planner.py`, register the three verbs in the one subcommand table at `:145-151`
...
- [x] 5.6 Assert the verbs write nothing into the deployment: a harness case planning the folder's
...
- [x] 6.1 Write `modules/provisioning/` as a machine module carrying exactly the facts
...
- [x] 6.2 In `modules/provisioning/`, assert the two facts the module cannot create, each naming
...
- [x] 6.3 Make `tests/e2e/guest.nix` a consumer: import the published module, hand it the account,
...
- [x] 6.4 Drop `pkgs.headscale` from the guest's system profile and its assertion
...
- [x] 6.5 Prove the module provisions a machine it did not write: one `tests/unit/modules.nix` case
...
- [x] 7.1 Hand the published leaf module to every folder as an argument the way `operator` is
...
- [x] 7.2 Rewrite the folder's membership phases as verbs: the credential is minted by the verb of
  ...
  it asserted.
- [x] 7.3 Keep what only a test needs and assert it is all that is left: the throwaway presenter
...
- [x] 7.4 Verify the folder still holds the enrollment invariants it landed with: a plan key
...
- [x] 8.1 In `docs/operator.md`, state the enrollment order of work as verbs an operator types:
...
- [x] 8.2 In `docs/operator.md`, state the provisioning declaration beside the invariant it serves
...
- [x] 8.3 In `docs/cluster.md`, state that the guest is a consumer of the published provisioning
...
- [x] 9.1 `nix eval --json '.#debug.failures'` returns `[]`; `nix build
...
- [x] 9.2 In **one** edit, now that every scenario has its test: delete this change's five
...
- [x] 9.3 Add this change's invariants to `CLAUDE.md`: under "Enrollment and the mesh", that a
