...
| `resolution` | 53 | wiring, arity, secrecy, placement, keyset equality |
| `diagnostics` | 48 | totality, ordering, severity, rendering, the scan that finds no raising call under `lib/` or in the reading beside it, the row table against `docs/diagnostics.md`, and every realiser refusal against the row producer above it |
| `plan` | 55 | keys, planes, absences, dependencies, serialisation, the golden fixture |
...
| `image` | 42 | the portable-service-image realiser's reading of an entry |
| `flakelet` | 21 | the flakelet realiser's reading: enablement, identity, the two name refusals, which host paths it carries |
| `operator` | 37 | the deployment build's reading: the artifact name, `manifest.json`, the realiser statement, its refusals |
...
523 tests, counted as the test attributes of the files under `tests/unit/`. Both
