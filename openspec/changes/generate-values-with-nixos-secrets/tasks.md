...
- [x] 1.1 Write `tests/e2e/generated-secret/deployment/` - three machines (`alpha`, `beta`,
...
- [x] 2.1 Add `tests/e2e/generation.py` with a pure half: given a `SecretsConfiguration` and a
...
- [x] 2.2 Implement the backend calls: `exists` per declared file, `get` only where the plan records
...
- [x] 2.3 Implement generation: resolve the tool from `$NIXOS_SECRETS_FLAKE` with `--refresh` the way
...
- [x] 2.4 Implement the provenance record and comparison per design D5: write each stored value's
...
- [x] 2.5 Probe the sandbox `bubblewrap` needs and skip the folder with a reason naming the missing
  kernel feature. Verify the skip reason is printed by `pytest.ini`'s `-rs`.
- [x] 2.6 Declare a prompt backend that fails on any invocation. Verify a configuration that would
  ask a prompt fails naming the value rather than blocking.

## 3. The declaration

- [x] 3.1 Add the program to `generatorKeys` in `lib/module.nix`, with a row for a declaration that
...
- [x] 3.2 Carry the program on the value's entry in `lib/plan.nix`, absent where none was declared.
  Verify a nix-unit test reads it off the entry and a second asserts an entry without one carries
  no such key.
- [x] 3.3 Exempt the program from the closure scan in `lib/plan.nix`. Verify a nix-unit test that a
  declared program produces no undeclared-mention row and appears in no entry's closure.
- [x] 3.4 Confirm the worked fixture declares no program, so the golden plan and
...
- [x] 4.1 Add `secrets/read.nix`: one store entry per generated value of a plan, its declared files,
...
- [x] 4.2 Implement the name projection and its three refusals - a colliding pair, a component
  carrying the separator, a component outside `safe-name` - each naming both sides. Verify one
  nix-unit test per refusal.
- [x] 4.3 Mark an undeployed value's files as not to be deployed, and refuse an entry with no
  program naming the entry and the field. Verify with two nix-unit tests.
- [x] 4.4 Add `secrets/backend.nix` rendering `deploy.remote` from the plan: per file, the delivery
...
- [x] 4.5 Record the pinned revision and the digest
...
- [x] 5.1 Rename the `secrets` unit suite to `vars`: `tests/unit/secrets.nix` to
...
- [x] 5.2 Add `tests/unit/secrets.nix` for the reading and register it in `tests/default.nix`.
  Verify `nix build .#checks.x86_64-linux.planner-tests` runs it.
- [x] 5.3 Add `secrets` to `classOf` in `tests/unit/layers.nix` as a realiser. Verify
  `layers.testEveryTopLevelEntryBelongsToAStatedClass` passes.
- [x] 5.4 Register the five spec files in `accountable` in `tests/unit/coverage.nix`. Verify
  `coverage.testEverySpecificationIsClassified` and `coverage.testAScenarioGainsNoTest` pass.

## 6. Documentation

- [x] 6.1 Write `docs/secrets.md`: the seam, the projection, the rendered deploy script, the pinned
...
- [x] 6.2 Document the program field in `docs/authoring.md` and `docs/plan.md` - the declaration
...
- [x] 6.3 Add the invariants to `CLAUDE.md`: the program is recorded and never run, it is no
...
- [x] 7.1 `nix build -L .#checks.x86_64-linux.planner-tests` - every suite, including the renamed
  `vars` and the new `secrets`.
- [x] 7.2 `nix build -L .#checks.x86_64-linux.planner-perf .#checks.x86_64-linux.planner-perf-checker` -
  re-record `perf/budgets.json` only if the added field moved a counter, and state why in the
  `note` field.
- [x] 7.3 `nix run .#planner-e2e generated-secret` - the new folder against real machines, with a
  real generator and a real store backend.
- [x] 7.4 `nix run .#planner-e2e` - all three folders, and `nix build -L .#checks.x86_64-linux.treefmt`.
