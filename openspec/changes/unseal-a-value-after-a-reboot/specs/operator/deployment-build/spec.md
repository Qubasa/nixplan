...
- The recipient is read with `machineRecordOf` the way the address is (`operator/read.nix:138-140`),
  off the `machine:<name>` record this change's planner half publishes it on, as an explicit
  absence: a line the atom refused was left out of every projection, so this reading is never
  handed a malformed one, and a machine without one is a warning the planner already produced
  (`machine-receives-a-value-unsealed`) - this reading adds no row of its own about it.
- The machine's scope is `run-an-entry-without-root`'s registry key, read off the machine record
  where that change records it; a machine with no recorded scope is system scope. Lingering, which
  makes a user manager a boot-time one, is that change's preflight fact and is not restated here.
-->
...
For every machine a delivered value reaches and whose record states a seal recipient, the build
SHALL produce one artifact carrying the program that opens a seal and the boot-time unit that runs
it, addressed by that machine's own name. The artifact SHALL be a function of the value file
records delivered to that machine and of the machine's scope, and of nothing else - not of the
recipient, so that a rotation rebuilds nothing: two builds of one deployment SHALL produce the same
one, and a change to one machine's values SHALL leave every other machine's artifact unchanged.
...
The unit SHALL be for the manager the machine's scope names: a system unit wanted by the system
manager's boot target on a system-scope machine, and a user unit wanted by the account manager's
default target on a user-scope one, running as the account, which owns the identity file there.
...
- **WHEN** a deployment delivers a value to a machine whose record states a seal recipient
...
#### Scenario: A user-scope machine receives a user unit

- **WHEN** a value is delivered to a machine whose recorded scope is user and whose record states a
  recipient
- **THEN** that machine's artifact SHALL carry a user unit wanted by the account manager's default
  target
- **AND** the unit SHALL run as the account rather than as root
...
machine, stating whether that machine's values are sealed, the machine's scope and, where they
seal, the artifact that opens them. The table SHALL be present whether or not it holds anything, so
that a reader cannot mistake a deployment that delivers nothing for a record written before the
table existed.

The record SHALL NOT restate the recipient the seal is made to: the plan's own machine record
carries it, and a second copy in the build record would be a second answer a stale build could
disagree with.
