...
- [ ] 1.1 Record `impl` only where the declaration carried a function, so the value the reading keeps
...
- [ ] 1.2 Force a guarded implementation once: read the recovery's own answer before any operand
  ...
  beside `impl-missing`.
- [ ] 1.3 Read a root's member container for its kind before it is indexed. Verify: the probe
  `aRootReturningServicesOfAnotherKindIsARow` answers `ok`.
- [ ] 1.4 Read each fragment of a rendered configuration file for its kind before it is coerced.
  Verify: the probe `aRecipeFragmentHoldingANonStringIsARow` answers `ok`.
- [ ] 1.5 Verify every settings value against its declared type before it enters a key input, and
...
- [ ] 1.6 Recognise a generated file reference by its record rather than by an attribute name, so a
  hand-written export shaped like one is a row. Verify: the probe
  `anExportCarryingTheVarsFileMarkerIsARow` answers `ok`.
- [ ] 1.7 Read the four arguments the entry point is handed - the machine registry, the instance
...
- [ ] 1.8 Add the suite assertions for 1.1 to 1.7 in `tests/unit/diagnostics.nix`, one per condition,
...
- [ ] 2.1 Produce each entry's row list under the recovery, so the table and the applicability
  ...
  raises.
- [ ] 2.2 Rewrite the two counterexamples that assert the withdrawn claim -
...
- [ ] 2.3 State the contract that a module's implementation accepts the arguments it does not name,
...
- [ ] 3.1 Refuse an empty name and a name carrying a line break or a control character where the name
...
- [ ] 3.2 Refuse a plan key claimed twice with an error row naming both claimants, and make the
...
- [ ] 3.3 Replace keying by what a declaration stated with a comparison against the value's own
...
- [ ] 3.4 Delete the record's statement of which ownership keys were written once nothing reads it,
...
- [ ] 3.5 Compare shown host paths of one machine for nesting as well as for equality, as an error
...
- [ ] 3.6 Add the remaining suite assertions for the scenarios of
  `specs/planner/plan-artifact/spec.md`. Verify: `nix eval --json .#debug.failuresBySuite.plan` is
  `[]`.

## 4. Diagnostics discipline

- [ ] 4.1 Apply the one-line rule to a row's subject, and make one definition of a line break -
...
- [ ] 4.2 Accept a subject that is a plan key the planner built, a path relative to the deployment
...
- [ ] 4.3 Deduplicate on what a producer built rather than on what a subject repair left, so two facts
...
- [ ] 4.4 Close the severity domain where a row is built: a value outside it is reported naming its
...
- [ ] 4.5 Give a fold's refusal a marker a successful fold cannot imitate, keeping the message, the
...
- [ ] 4.6 Add the suite assertions for the remaining scenarios of
...
- [ ] 5.1 Compare a secret export's secrecy against the secrecy of the file backing it, as an error
  ...
  in the plan.
- [ ] 5.2 Ask the readability comparison about an entry's own generated value, beside the two sites
...
- [ ] 5.3 Widen the mention scan from the entry's own undeployed values to every value the mentioning
...
- [ ] 5.4 Verify each delivered read against the consuming interface's own export type, leaving a
...
- [ ] 5.5 Reach every row an interface can earn from the modules that imported it rather than from
...
- [ ] 5.6 Add the suite assertions for the scenarios of `specs/planner/secret-delivery/spec.md`,
...
- [ ] 6.1 Read a configuration file's stated source for its kind and hold it to the store; record no
...
- [ ] 6.2 Refuse the bind-list separator and the specifier character in a shown host path, in the one
  home that grammar already has. Verify:
  `counterexamples.testAConfigurationFilePathIsAWordAUnitFileCanBind` passes.
- [ ] 6.3 Hold a unit's own name and an environment name to the rule their values are, and hold an
...
- [ ] 6.4 Carry the confinement profile in the image's version digest. Verify:
...
- [ ] 6.5 Escape an environment name the way its value is escaped, and refuse a part the renderer
  cannot carry. Verify: `counterexamples.testAnEnvironmentNameIsEscapedTheWayItsValueIs` passes.
- [ ] 6.6 Anchor the flakelet realiser's restated unit rule so no character the endpoint treats as a
...
- [ ] 6.7 Check every value the rendered secrets step carries as a word in the half that answers the
...
- [ ] 6.8 Classify a plan record in the secrets reading by what it records rather than by the text of
...
- [ ] 6.9 Index the derived unit file names per machine in the reading that derives them, as an error
  ...
  realiser.
- [ ] 6.10 Read the realisation statement's realiser and profile, and a plan's configuration file
...
- [ ] 6.11 Add the suite assertions for the scenarios of `specs/planner/unit-vocabulary/spec.md`,
...
- [ ] 7.1 Derive the readers a moved value restarts from the same relation the activation order is
  derived from, so every recorded read shape rotates its consumer. Verify:
  `pytest cli/counterexample_test.py -k rotates` passes.
- [ ] 7.2 Refuse a resolved read recorded in a shape the walk does not recognise whatever kind of
  value it is, naming the consumer and the slot. Verify: `pytest cli/counterexample_test.py -k
  neither_shape` passes.
- [ ] 7.3 Require a stated artifact path to resolve inside the build, naming the entry and the path
  otherwise, before the first dial. Verify: `pytest cli/counterexample_test.py -k leaves_the_build`
  passes.
- [ ] 7.4 Measure a value source to the leaves of each delivered value's own directory, naming every
  undeclared file by its path below it. Verify: `pytest cli/counterexample_test.py -k undeclared`
  passes.
- [ ] 7.5 Name a refusal from the record the command read rather than from the vector it built.
  Verify: `pytest cli/counterexample_test.py -k ssh_option` passes.
- [ ] 7.6 Produce the absence line only from an endpoint that answered and registered no entry, and
...
- [ ] 7.7 Compare an image by the identity the build published, selected out of the machine's answer
...
- [ ] 7.8 Add the tests for the new scenarios of `specs/operator/apply-command/spec.md` and
...
- [ ] 8.1 Count `cli/counterexample_test.py` as a layer of tests in the specification cross-walk, the
...
- [ ] 8.2 Move this change's 14 spec files from `excused` to `accountable` in
  `tests/unit/coverage.nix`. Verify: the `coverage` suite reports no unclassified, no vanished, no
  doubled and no untested scenario.
- [ ] 8.3 Record the new identifiers in `docs/diagnostics.md`, each under the family it belongs to.
  Verify: the row table cross-walk in `tests/unit/diagnostics.nix` accounts for every identifier the
  library produces.
- [ ] 8.4 Update the suite figures and the counterexample rows in `docs/tooling.md`, and the totals
  in `tests/unit/layers.nix`. Verify: `nix eval --json .#debug.failuresBySuite.layers` is `[]`.
- [ ] 8.5 Record in `CLAUDE.md`: the narrowed totality sentence and the two named conditions, the key
...
- [ ] 8.6 Re-measure the evaluation budgets for the widened mention scan and the added verify pass,
...
- [ ] 8.7 Regenerate the golden plan if and only if it moved, and state which rule moved it. Verify:
  `nix eval --json .#debug.worked.plan | jq -S .` equals `fixtures/minimal-typed-edge/plan/`'s
  committed files.

## 9. The whole tree

- [ ] 9.1 Run the full unit layer and account for every failure. Verify:
  `nix build .#checks.x86_64-linux.planner-tests` reports every test successful.
- [ ] 9.2 Run both counterexample checks and the command's tests. Verify:
  ...
  `ok`.
- [ ] 9.3 Run the machine layer on real guests, since the digest, the key input and the wire verify
