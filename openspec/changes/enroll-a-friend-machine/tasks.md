...
- [ ] 1.1 Register this change's one delta spec in `excused` in `tests/unit/coverage.nix`:
...
- [ ] 1.3 `git add` every new file of this change before evaluating anything - the flake does not
  see an untracked path. Verify with `nix eval --json '.#debug.failures'` returning something
  other than a file-not-found error.
- [ ] 1.4 Do not start this change before the four production changes have landed:
...
- [ ] 2.1 Add headscale and tailscale to `tests/e2e/guest.nix`, then run `rookery snapshot gc
  ...
  cold cut.
- [ ] 2.2 Write the hub's headscale service as a leaf module of the new folder's deployment: a
...
- [ ] 3.1 Declare the credential generator on the hub entry: a single-use, expiring pre-auth key,
...
- [ ] 3.2 Prove the argv discipline: a `tests/e2e/test_harness.py` case over the recorded argv of
...
- [ ] 4.1 Add `tests/e2e/friend-enrollment/`, discovered from its `deployment/default.nix` like
...
- [ ] 4.2 Phase: the friend machine joins the mesh with the credential; the hub's node list names
  it, echoed as `key=value` lines through one ssh command per case.
- [ ] 4.3 Phase: a second join presenting the same credential is refused by the server, and the
  node list still names the first machine alone.
- [ ] 4.4 Phase: an apply dials the friend machine by its mesh name - the user-scope preflight
  runs first, the entry activates under the account's manager, and the report reads the holdings
  over the same name.
- [ ] 4.5 Phase: the operator expires the friend machine's node; a report names the machine as one
  it could not ask and exits non-zero. Run this phase last - it removes the wire the earlier
  phases stand on.

## 5. Documents and close

- [ ] 5.1 In `docs/operator.md`, state the enrollment order of work: declare the row, generate,
...
- [ ] 5.2 In **one** edit: move this change's path from `excused` to `accountable` in
...
- [ ] 5.3 Gates: `nix eval --json '.#debug.failures'` returns `[]`, `planner-tests` and `treefmt`
