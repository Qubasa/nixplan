...
- An image carries no generated bytes, and it carries the bytes of a configuration file only where
  the plan holds them: a `source` path and a literal recipe are store objects the image's closure
  reaches, and a `ref`-bearing recipe arrives from the host at attach time. An edit to a literal
  file therefore moves the image's version digest and an edit to a referenced one does not.
- The attach script assembles a `ref`-bearing configuration file into a temporary the `install -m`
...
- flakelet decides `[Install]`, no plan field does. A long-running unit is wanted by
  ...
  its schedule.
- The flakelet realiser refuses a shown host path by **when its bytes exist**, not by what kind of
  file it is: it runs no step on the machine, so a recipe carrying a `ref` is the one case it
  cannot carry. A delivered generated file arrives before activation, a `source` file is already a
  store object, and a recipe of nothing but literals is assembled at build time and carried in the
  artifact beside `meta.json` and `units/`.
- Both realisers are handed the same `assemble` argument by the reading, so a configuration file
  ...
  apply replace it - and only a `ref`-bearing recipe is staged by the attach step.
...
- `shared-postgres`'s server reads one `configData` file of literals, which the flakelet realiser
  assembles at build time and carries in the artifact. The data directory stays state rather than
  `configData`, because a realiser writes bytes and not a directory a server initialises.
- No unit of it is root. The password is delivered `postgres:postgres 0440`, the cluster's two units
