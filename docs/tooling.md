...
| `composition` | 40 | roots, members, the settings namespace, defaults and fixed |
| `resolution` | 62 | wiring, arity, secrecy, placement, keyset equality |
| `diagnostics` | 59 | totality, ordering, severity, rendering, the scan that finds no raising call under `lib/` or in the reading beside it, the row table against `docs/diagnostics.md`, and every realiser refusal against the row producer above it |
...
| `units` | 47 | the portable unit vocabulary, per field |
| `platform` | 26 | a machine's target, elaborated |
| `closure` | 25 | what a unit may name and what it must declare |
| `image` | 53 | the portable-service-image realiser's reading of an entry |
...
| `layers` | 29 | the shape of the test tree itself, the root document and the one shell |
...
610 tests, counted as the test attributes of the files under `tests/unit/`. Both
...
`nix build .#checks.x86_64-linux.planner-delivery -L`, 90 tests over
...
`PYTHONPATH=cli pytest -q tests/e2e/test_harness.py` runs the same 90 against
