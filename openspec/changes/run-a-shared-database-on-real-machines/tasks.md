...
- [x] 1.1 List this change's two spec files under `excused` in `tests/unit/coverage.nix` with the
...
- [x] 1.2 Add `users.users.postgres` (and its group) to `tests/e2e/guest.nix` with an assertion
...
- [x] 1.3 Record the orphaned cuts before and after with `rookery snapshot list`, run
  `rookery snapshot gc --all`, and verify one folder resumes cold and then warm on a second run

## 2. The deployment

- [x] 2.1 Write `tests/e2e/shared-postgres/deployment/interfaces/default.nix` declaring one
...
- [x] 2.2 Write `deployment/modules/postgresql/databases.nix`: a leaf with a `fixed` port claim, one
...
- [x] 2.3 Write `deployment/modules/postgresql/cluster.nix`: a root over the one member, re-exporting
...
- [x] 2.4 Write the two units in the leaf's `impl` - a root `oneShot` initialiser with
...
- [x] 2.5 Write the initialising program as a package the entry declares in `closure`: initialise the
...
- [x] 2.6 Write `deployment/modules/app/` - the consumer module declaring one slot reading
...
- [x] 2.7 Write `deployment/machines.nix` (two machines at the cluster's static addresses) and
...
- [x] 2.8 Write `deployment/default.nix` taking `pkgs`, `planner` and `operator`, returning one
...
- [x] 2.9 Verify the folder is discovered without editing the flake: `nix eval` the package name and
  confirm `testAnEndToEndFolderIsAddedWithoutEditingTheFlake` still passes

## 3. The machine layer

- [x] 3.1 Declare the folder's stage through `delivery.cluster_stage` with two slots, `uefi = true`,
...
- [x] 3.2 Add the build, value-source and apply fixtures: build in the pytest process, mint two
  ...
  three activations
- [x] 3.3 Verify the apply order: the cluster is activated before either consumer, read off the
  recorded step lines

## 4. The assertions

- [x] 4.1 `test_two_instances_take_one_database_each`: the plan carries three entries, each
  consumer's unit carries its own `dsn` and not the other's
- [x] 4.2 `test_a_consumer_on_another_machine_reads_over_the_address_the_plan_recorded`: the far
  consumer's record names the planned address and port, and the row it wrote is readable
- [x] 4.3 `test_one_process_is_behind_both_capabilities`: one `MainPID` for the server, both
  databases answered by it, and the initialiser reported as an exited one-shot
- [x] 4.4 `test_a_credential_of_one_capability_is_refused_by_the_other`: presenting the `eu`
  credential to the `us` database is refused by the server, with the server's own message
- [x] 4.5 `test_a_consumers_machine_holds_its_own_credential_only` and
...
- [x] 4.6 `test_data_written_before_a_restart_is_readable_after_it`: restart the long-running unit,
  read the row back, and confirm the initialiser's start timestamp did not move
- [x] 4.7 Verify no password appears in the build tree, in `plan.json`, or in any machine's store:
  grep the built farm and `/nix/store` on both machines
- [x] 4.8 Verify each test is one ssh command reporting `key=value` lines, and that the whole folder
...
- [x] 5.1 Add the folder's invariants to `CLAUDE.md`: why the server takes flags rather than a
...
- [x] 5.2 Verify `tests/e2e/shared-postgres` is picked up by the treefmt mypy run whose module list
  is read from the directory, and that `nix build .#checks.x86_64-linux.treefmt` passes
- [x] 5.3 Run the whole end-to-end layer once and verify no other folder regressed from the image
