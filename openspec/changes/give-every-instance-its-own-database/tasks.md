...
- [ ] 1.1 In `tests/e2e/shared-postgres/deployment/modules/postgresql/databases.nix`, take `instance`
...
- [ ] 1.2 Change `fixed.port` to `defaults.port = 5432` in
...
- [ ] 1.3 In `.../modules/app/client.nix`, derive the record path (`/run/<name>/record`) and the
  ...
  directories.
- [ ] 1.4 Verify no `.nix` file under `tests/e2e/shared-postgres/deployment/` carries an
...
- [ ] 2.1 Add the tag that places the private instance to `alpha` in
  `.../deployment/machines.nix`, and verify `beta`'s tags are unchanged.
- [ ] 2.2 In `.../deployment/instances.nix`, place both `own-app` members on that tag, state
  ...
  on `beta`.
- [ ] 2.3 Verify the deployment is applicable and its diagnostics table carries no error row, and in
...
- [ ] 3.1 Delete `NEAR_RECORD`, `FAR_RECORD`, `OWN_RECORD`, `DATA_DIR`, `OWN_DATA_DIR` and
...
- [ ] 3.2 Move the private instance's keys and machine: `OWN_KEY`, `OWN_DB_KEY` and the delivery set
...
- [ ] 3.3 Raise `DISK_GIB` for the machine now holding two data directories and verify the stage
  resumes: the first run is cold, `rookery snapshot list` shows one new entry for this folder and
  none for another.
- [ ] 3.4 Add `test_two_instances_of_one_module_run_on_one_machine`: one ssh command reporting both
...
- [ ] 3.5 Add `test_every_path_a_unit_uses_comes_from_the_plan`: every path asserted in this module is
  read from the plan, each contains its entry's instance and member, and the deployment's own files
  state none of them.
- [ ] 3.6 Add `test_each_application_reaches_the_database_it_wired`: each consumer's record names the
  identity of the cluster it wired, the two identities differ, and each row is readable in its own
  cluster.
- [ ] 3.7 Add `test_neither_cluster_carries_the_others_databases`: each server's `datname` list read
  over its own socket, asserted against what the deployment declared for it.
- [ ] 3.8 Add `test_a_credential_of_the_shared_cluster_is_refused_by_the_private_one`: the shared
  instance's password presented to the private instance's port, and the server's own refusal read
  back.
- [ ] 3.9 Add `test_the_private_clusters_credential_is_on_its_own_machine_only`: present on `alpha`,
  absent on `beta`, and the plan's delivery record naming `alpha` alone.
- [ ] 3.10 Verify every case is one ssh command whose output is `key=value` lines, and that the
  existing eight tests of the folder still pass unchanged in meaning.

## 4. The layer's own checks

- [ ] 4.1 Keep `statefulFolders` in `tests/unit/layers.nix:700` recognising this folder once
...
- [ ] 4.2 Add `testAStatefulFolderDeclaresItsSpaceOnItsOwnStage` and
...
- [ ] 4.3 Move this change's two spec files from `excused` to `accountable` in
...
- [ ] 5.2 Run `nix run .#planner-e2e shared-postgres` and verify every test passes on real machines,
  including the four new ones; record the wall time of the cold run.
- [ ] 5.3 Update `CLAUDE.md`'s `shared-postgres` invariants: the port is a default and why, every host
...
- [ ] 5.4 Run `nix build .#checks.x86_64-linux.treefmt` and verify formatters, `ruff`, `mypy` and
  `vale` are green.
