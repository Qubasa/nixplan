...
- [ ] 1.1 Add the rule to `README.md` beside the design goals at `README.md:17-20`: a deployment
...
- [ ] 1.2 Record the same rule in `CLAUDE.md` under a heading of its own, naming the two checks that
  hold it and the two exemptions (a test's own fake root; the unit suites' fixtures).

## 2. The checks

- [ ] 2.1 Add the host-path scan to `tests/unit/layers.nix`: over every `.nix` file under
...
- [ ] 2.2 Add `testAUrlPathIsNotAHostPath` and `testADerivedPathIsPermitted`, asserting the scan's
  own classification over the two shapes, including `tests/e2e/wired-pair/deployment/default.nix`'s
  `"/index.html"`.
- [ ] 2.3 Add the per-folder intersection check and `testATestRestatesAPathItsDeploymentCarries` plus
...
- [ ] 2.4 Add `testTheRootDocumentStatesHowAPathReachesAUnit`, reading a literal of the rule out of
  `README.md`, and verify it is red with that sentence deleted.

## 3. The folders

- [ ] 3.1 `tests/e2e/wired-pair/`: derive the probe's record path and the sweep's marker path from
...
- [ ] 3.2 `tests/e2e/secret-delivery/`: the same for the probe and the idle job, and delete the
...
- [ ] 3.3 `tests/e2e/generated-secret/`: the same for its probe and idle job. Verify the two plans the
...
- [ ] 3.4 `tests/e2e/portable-image/`: derive the shown, assembled and quiet paths in
...
- [ ] 3.5 `tests/e2e/newcomer/template/`: derive the greeting path in `modules/hello/greet.nix`,
  ...
  are green.
- [ ] 3.6 `tests/e2e/newcomer/test_newcomer.py`: read the greeting path out of the plan the
...
- [ ] 4.1 Move this change's two spec files from `excused` to `accountable` in
...
- [ ] 4.3 Run `nix run .#planner-e2e` and verify all six folders pass; record which folders were
  ...
  mistake.
- [ ] 4.4 Run `nix build .#checks.x86_64-linux.treefmt` and verify formatters, `ruff`, `mypy` and
  `vale` are green.
