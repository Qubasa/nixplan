...
- [ ] 1.1 Confirm the change directory and its diff consume nothing from any parked change: no
  ...
  implementation.
- [ ] 1.2 Read the machine's scope where `run-an-entry-without-root` records it and add none of it:
...
- [ ] 1.3 Take no decision that belongs to another open change: the retire step's place in the walk
...
- [ ] 2.1 Record the perf baseline before editing anything under `lib/`: run `bash perf/measure.sh`
  ...
  budget.
- [ ] 2.2 Confirm this change's six delta specs are in `excused` in `tests/unit/coverage.nix`, one
...
- [ ] 3.1 Add `ageRecipientRule` to `lib/util.nix` beside `wordRule` (`lib/util.nix:129-167`): `age1`
...
- [ ] 3.2 Add the `ageRecipient` atom to `lib/atoms.nix`, reading `ageRecipientRule` the way
  `restartPolicy` reads `restartPolicies` (`lib/atoms.nix:17-101`): the grammar has one home and the
  atom and the row both read it.
- [ ] 3.3 In `lib/resolve.nix`, read `sealRecipient` in the third projection of the machine reading,
...
- [ ] 3.4 In `lib/plan.nix`, record the recipient on the `machine:<name>` record
...
- [ ] 3.5 In `lib/plan.nix`, produce `machine-receives-a-value-unsealed` where the delivery set is
...
- [ ] 3.6 In `tests/unit/platform.nix`, add the nix-unit tests named after the machine-platform delta's
  ...
  still planned).
- [ ] 3.7 In `tests/unit/resolution.nix`, add `testAMachineInADeliverySetStatesNoRecipient` and
...
- [ ] 4.1 Add `sealedRoot = "/var/lib/planner/sealed"` immediately beside `varsRoot`
  ...
  '.#debug.failures'` returning `[]`.
- [ ] 4.2 In `tests/unit/vars.nix`, add `testAPersistentPathIsDerivedFromTheValuesOwnPath` (nix-unit
...
- [ ] 4.3 In `tests/unit/vars.nix`, add `testAPersistentPathIsNotAValuePathToTheScanThatRecognisesOne`
...
- [ ] 4.4 In `tests/unit/vars.nix`, add `testAPlanIsUnchangedByTheDerivationOfAPersistentPath`
...
- [ ] 4.5 In `tests/unit/vars.nix`, add `testARotatedIdentityReKeysNothing` (nix-unit layer): plan a
...
- [ ] 5.1 Add the per-machine half of the reading beside the per-entry one: for every machine a
...
- [ ] 5.2 Derive the ordering list from `planner.util.varsPathsDeep` of each placed entry's own
...
- [ ] 5.3 Publish the `machines` table in the manifest, one record per machine a delivered value
...
- [ ] 5.4 In `tests/unit/operator.nix`, add these nix-unit tests, one per scenario of
...
- [ ] 6.1 Build one artifact per machine of the reading's per-machine record and put it in the same
...
- [ ] 6.2 `bin/unseal`: for each value file in plan key order, leave a plaintext that is already
...
- [ ] 6.3 `bin/check`: per sealed copy, print whether the copy is there and whether it opens, and
...
- [ ] 6.4 `planner-unseal.service`: a oneshot running `bin/unseal`, `After=local-fs.target`, and
...
- [ ] 7.1 Add two file-scope entries to `renderedWords` (`secrets/read.nix:280-324`): the sealed path
...
- [ ] 7.2 Add the recipient as a per-machine rendered word, read off the plan's `machine:<name>`
...
- [ ] 7.3 Give `render` one more caller argument beside `get`: the sealing program, because the plan
  holds no store path to a tool.
- [ ] 7.4 Render the sealing half: after the backend's `get` has written the plaintext into the
  ...
  single `deliver`.
- [ ] 7.5 In `tests/unit/secrets.nix`, add `testTheRenderedStepWritesASealedCopyForAMachineThatSeals`
...
- [ ] 8.1 `cli/manifest.py`: implement record version 2, read the required `machines` table, and
...
- [ ] 8.2 `cli/flake-module.nix`: name the sealing program in the command's own wrapper, off the
...
- [ ] 8.3 `cli/apply.py`: before the first value of a machine is written, copy that machine's
...
- [ ] 8.4 `cli/remote.py` and `cli/apply.py`: seal in the process that read the value source and send
...
- [ ] 8.5 `cli/values.py` or `cli/apply.py`: refuse before the first dial where the record says a
  machine seals and the invocation names no sealing program, naming the program and the machines.
- [ ] 8.6 `cli/report.py`: extend the one per-machine value question to the machine's own `bin/check`
...
- [ ] 8.7 In `tests/e2e/test_harness.py`, add the pytest tests for the scenarios the recorder can
...
- [ ] 9.1 Generate the throwaway identity once, at authoring time, and commit both halves: the
...
- [ ] 9.2 Add the folder's first phase: provision each machine the way an operator would,
...
- [ ] 9.3 Add to the `applied` phase, after
...
- [ ] 9.4 Add `test_a_machine_holding_a_sealed_copy_it_cannot_open`: replace `beta`'s sealed copy of
...
- [ ] 9.5 Add `test_a_machine_that_seals_and_holds_no_sealed_copy`, and rewrite the body of
...
- [ ] 9.6 Add `test_a_copy_the_machine_cannot_open_leaves_the_value_absent_and_names_it`: clear one
...
- [ ] 9.7 Rewrite the `rebooted` phase's test into
...
- [ ] 9.8 In `tests/e2e/delivery.py`, add the sealing-tool guard beside `endpoint_refusal`, and
...
- [ ] 9.9 Check whether `tests/e2e/secret-delivery/`'s stage needs more disk for the sealed copies
...
- [ ] 9.10 Check the cross-walk by hand before 11.4: every new test of sections 3 to 9 is named the
...
- [ ] 10.1 `docs/operator.md`: the unsealer install step in the apply's step list, the sealed write
...
- [ ] 10.2 `docs/cluster.md`: the provisioning phase and the new phases of
...
- [ ] 11.1 Regenerate the golden fixture: `fixtures/minimal-typed-edge`'s declarations do not
...
- [ ] 11.2 Run `nix build .#checks.x86_64-linux.planner-perf`. The gate is two-sided with a 0.15
  margin: compare against the counters recorded in 2.1 and fix a movement rather than re-recording a
  budget.
- [ ] 11.3 Run `nix eval --json '.#debug.failures'` and require `[]`, then `nix build
...
- [ ] 11.4 In **one** edit, now that every heading of this change's six delta specs has its test in
...
- [ ] 11.5 Add this change's invariants to `CLAUDE.md`, and name the hand-maintained registration
