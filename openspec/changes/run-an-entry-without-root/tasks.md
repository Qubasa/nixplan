...
- [ ] 1.2 Register this change's five delta specs in `excused` in `tests/unit/coverage.nix`, one
...
- [ ] 2.1 In `lib/atoms.nix`, state `scopes = [ "system" "user" ]` beside `restartPolicies`
...
- [ ] 3.1 In `lib/resolve.nix`, read the optional `scope` key from the machine registry beside
  ...
  placement.
- [ ] 3.2 In `lib/resolve.nix`, enter `scope` into `machineRecords` and `targetOf` only when it is
...
- [ ] 4.1 In `lib/resolve.nix`, where `placement-platform-mismatch` is produced, add the three
  ...
  machine earn none.
- [ ] 4.2 In `lib/plan.nix`, add `value-ownership-in-user-scope`: a generated file record stating
...
- [ ] 4.3 Register the four identifiers in the row tables of `docs/diagnostics.md`, and confirm
...
- [ ] 5.1 In `image/read.nix`, publish `scopes = [ "system" "user" ]`, and in `flakelet/read.nix`
...
- [ ] 5.2 In `operator/read.nix`, cross the stated realiser's published `scopes` against the
...
- [ ] 5.3 In `flakelet/read.nix`, make the builder's own refusal for a user-scope target carry
...
- [ ] 6.1 In `image/default.nix` and `image/read.nix`, place unit files under
...
- [ ] 6.2 In `image/default.nix`, build the user-scope image as squashfs plus dm-verity with a
...
- [ ] 6.3 In `image/read.nix`, read the confinement profiles per scope: the user profiles drop
...
- [ ] 7.1 In `image/default.nix`, render the attach script for a user-scope entry with
...
- [ ] 8.1 In `cli/remote.py`, add the one preflight question per user-scope machine: one script
...
- [ ] 8.2 In `cli/apply.py`, ask the question before any mutation on that machine - before the
  ...
  run.
- [ ] 8.3 In `cli/remote.py` and `cli/report.py`, address a user-scope machine's manager and
...
- [ ] 9.1 Spike first: enable `systemd-mountfsd.socket`, `systemd-nsresourced.socket` and user
...
- [ ] 9.2 Add a user-scope end-to-end folder under `tests/e2e/`, discovered from its
  ...
  <folder>`.
- [ ] 9.3 Add the negative harness coverage that needs no machine: a flakelet entry stated on a
...
- [ ] 10.1 In `docs/operator.md`, state the provisioning invariant - root at provision time, never
...
- [ ] 10.2 In `docs/plan.md`, document `scope` under the machine record and the target - the
...
- [ ] 11.2 In **one** edit, now that every scenario has its test: delete this change's five
