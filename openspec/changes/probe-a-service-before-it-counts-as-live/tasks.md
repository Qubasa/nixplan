...
- [ ] 1.1 Record the perf baseline before editing anything under `lib/`: run `bash perf/measure.sh`
...
- [ ] 2.1 Add the zero-duration predicate to `lib/atoms.nix`, exported beside `portRange` and
...
- [ ] 2.2 Add `probe = atoms.string` and `probeTimeout = atoms.duration` to `unitVocabulary` in
...
- [ ] 2.3 Add the pair rows to `lib/module.nix`, behind one `unit ? probe || unit ? probeTimeout`
...
- [ ] 2.4 Add the shape rows to `lib/module.nix`: `unit-probe-on-one-shot` and
...
- [ ] 2.5 Add `unit-probe-declared-twice` to `lib/module.nix`, computed once per entry over the unit
...
- [ ] 2.6 Confirm by test rather than by reading that no scan needed extending: `closure-path-undeclared`
...
- [ ] 3.1 Add the derived probe file name to `unitFilesOf` in `image/read.nix`, as
...
- [ ] 3.2 Add `probe = "ExecStart"` and `probeTimeout = "TimeoutStartSec"` to `unitDirectives` in
...
- [ ] 3.3 Add `renderProbe` to `image/read.nix`: `Type=oneshot`, `After=` and `Requires=` the probed
...
- [ ] 3.4 Render it from `renderedUnitsBy` rather than from either realiser's wrapper, so the two
...
- [ ] 3.5 Add the probe file to `attachment.units` in `image/read.nix` from the same helper
  `unitFilesOf` uses, so the attach script's `systemctl start` starts it. Verify the attachment
  record in `tests/unit/image.nix`.
- [ ] 3.6 Add the four accounts of design D7 to `image/read.nix`'s `accounts` table and the refusals
...
- [ ] 3.7 Confirm `flakelet/read.nix` needs no edit: its `acceptsUnit` accepts the derived name, its
...
- [ ] 4.1 Add `operator-entry-probe-unit-file-taken` to `operator/read.nix`, an error naming the
...
- [ ] 5.1 Add every new identifier to the table in `docs/diagnostics.md`:
...
- [ ] 5.2 Add `probe` and `probeTimeout` to the unit vocabulary table in `docs/authoring.md`, beside
...
- [ ] 6.1 `tests/unit/units.nix` (nix-unit): `A unit that says how it is probed`, `A probe whose
...
- [ ] 6.2 `tests/unit/plan.nix` (nix-unit): `An entry that declares no probe keeps its key`, `The
  plan says how to probe and never whether it is healthy`, `A probe adds no unit to the plan`.
- [ ] 6.3 `tests/unit/closure.nix` (nix-unit): `A probe naming an undeclared store path`.
  `tests/unit/vars.nix` (nix-unit): `A probe naming a value the machine does not receive`.
- [ ] 6.4 `tests/unit/flakelet.nix` (nix-unit): `A probed entry carries one more unit file`, `The
...
- [ ] 6.5 `tests/unit/image.nix` (nix-unit): `An image of a probed entry carries the probe unit`, `A
  ...
  units`.
- [ ] 6.6 `tests/unit/operator.nix` (nix-unit): `A declared unit spelling the derived probe file`.
- [ ] 6.7 Verify the whole set with `nix build .#checks.x86_64-linux.planner-tests` and
...
- [ ] 7.1 Add a third deployment build to `tests/e2e/wired-pair/deployment/default.nix` whose served
...
- [ ] 7.2 Add the pytest phases to `tests/e2e/wired-pair/test_wired_pair.py`, **last in file order**
...
- [ ] 7.3 Add the image half to `tests/e2e/portable-image/`, also last in file order: a build whose
...
- [ ] 7.4 Leave `tests/e2e/shared-postgres/` alone. Its killed-main-process phase is the nearest
...
- [ ] 8.1 `fixtures/minimal-typed-edge` is **not** touched: no declaration of it gains a probe, so
  ...
  in the commit.
- [ ] 8.2 Run `nix build .#checks.x86_64-linux.planner-perf`. The gate is two-sided with a 0.15
...
- [ ] 8.3 Run `nix eval --json '.#debug.failures'` and require `[]`, then
  `nix build .#checks.x86_64-linux.planner-tests` and `nix build .#checks.x86_64-linux.treefmt`.
- [ ] 8.4 In **one** edit, now that every heading of task groups 6 and 7 has its test: delete this
...
- [ ] 8.5 Add this change's invariants to `CLAUDE.md`: under **Realisers**, that the probe is one
