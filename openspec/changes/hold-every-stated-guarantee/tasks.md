...
- [ ] 2.1 Write the failing check first: in `tests/unit/operator.nix` or the `cli` layer's own suite,
...
- [ ] 2.2 Teach `cli/order.py`'s `edges` the per-provider shape (`slot["entries"]`, an attrset keyed
...
- [ ] 2.3 Per design D1, make a delivered, resolved read whose shape `edges` does not recognise an
...
- [ ] 2.4 Add the tests the `operator/apply-command` scenario headings name for the set-read and
...
- [ ] 2.5 Move `hold-every-stated-guarantee/specs/operator/apply-command/spec.md` from `excused` to
...
- [ ] 3.1 Announce the rollback step before it is attempted: apply `cli/apply.py`'s `_taking`
...
- [ ] 3.2 Resolve every artifact a run will need - including `image_file(entry)`, today called inside
...
- [ ] 3.3 Route the last unguarded read in `cli/manifest.py` (`_rows`, line 362) through `_load`, and
      verify a built directory whose `diagnostics.json` is not JSON is refused by name rather than a
      `JSONDecodeError`.
- [ ] 3.4 Add the tests the two `Every refusal a run can make without a machine precedes the first
      machine` headings name, and verify each is red before its repair.

## 4. The record of an entry both sides agree on

- [ ] 4.1 Write the failing check first: a deployment placing one service that publishes an export
...
- [ ] 4.2 Decide the record in one place per design D10's sibling decision in the spec - an entry with
...
- [ ] 4.3 Make a command that needs an artifact for such an entry refuse naming that entry alone, and
...
- [ ] 4.4 Move `specs/operator/deployment-build/spec.md` to `accountable` and verify the coverage suite
      is `[]`.

## 5. The reading of a plan is total

- [ ] 5.1 Write the failing check first: a generator declaring no files is planned, and the plan is
...
- [ ] 5.2 Record `files` on a value entry whether or not it is empty, beside the `delivery` field
...
- [ ] 5.3 Per design D2, make `operator/read.nix` produce a row naming the entry and the field for any
      field the plan may omit, rather than reading it bare, and verify 5.1 now yields a table.
- [ ] 5.4 Add the tests the two `The reading is total over a plan the planner pruned` headings name,
      and verify each is red with 5.2 and 5.3 reverted.

## 6. `lib/` stops raising

- [ ] 6.1 Write the failing checks first, one per confirmed site: an instance with no `module`
...
- [ ] 6.2 Introduce the one guarded field reading of design D5 in `lib/resolve.nix` - a field read
...
- [ ] 6.3 Migrate all seven sites to it and verify every check from 6.1 is green, each naming the
      declaration and the field.
- [ ] 6.4 Move the export atom check to where the export is used and put it under the guard
...
- [ ] 6.5 Make the refusal channel accept only text (`lib/resolve.nix:1012-1018`): a `refused` that is
...
- [ ] 6.6 Verify `lib/` still holds its purity scan: `nix eval --json .#debug.failuresBySuite.diagnostics`
      is `[]` and no `throw`, `abort`, `assert`, `.check ` or `korora.check` was added under `lib/`.
- [ ] 6.7 Add the tests the `planner/diagnostics` headings name for the deployment half, the atom and
...
- [ ] 7.1 Write the failing check first: machines `alpha` and `be@ta`, an instance-wide generator
...
- [ ] 7.2 Refuse a machine, instance, member or generator name carrying `@`, `:` or `/` per design D3,
...
- [ ] 7.3 Write the failing check for the second spelling: `services.pg = service "postgres" { … }`
      today gives `error: attribute 'postgres' missing`, naming nothing.
- [ ] 7.4 Make the attribute key a member's identity per design D4: `lib/resolve.nix:536,923` read the
...
- [ ] 7.5 Reach interface rows from the modules rather than the attribution list per design D6
...
- [ ] 7.6 Add the tests the `planner/plan-artifact` headings name, move that spec path to
      `accountable`, and verify the coverage suite is `[]`.

## 8. The image stages a file at its declared mode, and the checks can fail

- [ ] 8.1 Write the failing check first: an image entry declaring
...
- [ ] 8.2 Create the staged file with its declared mode per design D7, and verify the mode is correct
...
- [ ] 8.3 Add the tests the three `realiser/portable-service-image` headings name, move that spec path
      to `accountable`, and verify the coverage suite is `[]`.
- [ ] 8.4 Replace the tautology at `tests/unit/vars.nix:872` with an observation of the property per
...
- [ ] 8.5 Replace `tests/unit/interfaces.nix:696`'s two applications of one pure function with the
      suite's `support.anotherEvaluation`, and verify it still passes for the right reason - two
      independent library evaluations.
- [ ] 8.6 Rebuild the e2e provenance guard from the declaration rather than the values the run just
...
- [ ] 8.7 Delete the implementation-echo expectations at `tests/unit/operator.nix:467-470,1134-1138`,
...
- [ ] 8.8 Make `perf/check.py` refuse a gated fixture with no measurement and report comparisons made
...
- [ ] 8.9 Add the tests the `tooling/test-layers` headings name, move that spec path to `accountable`,
...
- [ ] 10.1 `nix build -L --no-link .#checks.x86_64-linux.planner-tests`: green, and record the test
      count here against the 382 this change starts from.
- [ ] 10.2 Regenerate the golden once:
      `nix eval --json .#debug.worked.plan | jq -S . > fixtures/minimal-typed-edge/plan/backup.json`.
      Record here which entries moved and why - a new always-present `files` field on a value entry
      and any new row are the only expected differences - and verify no entry key moved for a reason
      this change did not state.
- [ ] 10.3 Re-record the perf budgets once, from `nix build .#packages.x86_64-linux.planner-perf-results`,
      with `check.py`'s own rounding, margin 0.15 and growth 1.25 untouched. Record the before/after
      cost per plan entry for every counter that moved, and remove any avoidable cost this change
      introduced before recording. Verify `.#checks.x86_64-linux.planner-perf` and
      `.#checks.x86_64-linux.planner-perf-checker` exit 0.
- [ ] 10.4 `nix build -L --no-link .#checks.x86_64-linux.planner-delivery` and
      `.#checks.x86_64-linux.treefmt`: green.
- [ ] 10.5 `nix run .#planner-e2e` with `PYTHONPATH` unset: green for every folder, count recorded
      here against the 75 this change starts from, and no folder skipped for a reason this change
      introduced.
- [ ] 10.6 Prove the change's own new checks can fail: for each of the nine repairs, revert the repair,
      observe the named check red, restore. Record the list here.
