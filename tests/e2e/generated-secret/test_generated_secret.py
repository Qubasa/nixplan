...
def generated(run: Run) -> Run:
    ...
    # A value the backend already holds was generated by an earlier run of this
    # folder, from whatever that run declared. Before this one generates
    # anything is the only point at which the two can disagree: after it, every
    # stored value is this declaration's by construction.
    generation.require_provenance(values, run.ungenerated, generation.read_provenance(record))
