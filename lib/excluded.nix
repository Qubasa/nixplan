# The exclusion table, as data. A deployment writing one of these keys gets an
# error row naming the construct and the trigger that would bring it back. row is
# the group of the table the construct belongs to, so a test can assert that
# every group of it is refused.
{
  rows = [
    "locality"
    "lifecycle"
