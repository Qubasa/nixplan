...
- `builtins.tryEval` catches a `throw` and a failed `assert`. It catches neither an abort, nor a
  ...
  loudly.
- A pattern is read before the function is applied, which is what narrows the rule above. An
  implementation whose argument pattern is closed is `implementation-formals-closed` and is not
  applied: `builtins.toXML` of a function prints that pattern and is the one place the ellipsis is
  observable, `builtins.functionArgs` answering the same record for `{ a }` and `{ a, ... }`. A
  slot nobody wired leaves the implementation unapplied too, so the entry is planned, records
  `units = { }` and the table prints. Three conditions stay uncatchable and are still documented as
  propagating: an abort, a missing attribute a module selects from its own expression, and a
  function called without an argument its pattern requires.
...
- An interface fold is the only channel through which a module refuses a value another module
  produced. The fold returns `planner.refuse "<why>"` and supplies the message; the planner
  supplies the identifier, the consuming entry as subject and the severity. A refusal is recognised
  by the marker attribute that constructor writes and by nothing else, so a fold's own successful
  result may carry an attribute named `refused` - which is what a partitioning fold calls the
  members it left out - and is delivered. `impl` gains no `refusals` key, and a module still may
  not tag a row's severity. `builtins.tryEval` reports that something raised and never what it
  said, which is why a refusal is a returned value and why `module-raised` carries the planner's
  text rather than the author's.
...
- A unit's environment names are held to their own grammar, `envNameRule`, and its home is
  `lib/util.nix` beside `keySeparators`, `wordRule` and `pathRule` for the reason the path grammar
  lives there: `unit-env-name-malformed` in `lib/module.nix` and the refusal `image/read.nix` makes
  read the one rule, because an assignment is rendered `Environment="<k>=<v>"` with no escape of
  the name, so a name outside the grammar is a directive of its own.
...
- A field enters a key only where its value differs from the default it would resolve to unstated.
  One projection in `lib/plan.nix`, `withoutDefaults ownershipDefaults`, holds that for the two
  file records this library carries: a generated file's `owner`, `group` and `mode`, and a
  configuration file's `owner` and `group`. Stating a default is therefore no re-key, whichever
  record states it, and a configuration file's `mode`, which has no default to sit at, is always in
  the key.
...
- An image's version digest is deliberately not the entry key. It is taken over what the artifact
  ...
  and a `ref`-bearing recipe's content moves the entry key without moving the image. The stated
  confinement profile is in that digest, because it decides directives the rendered unit files
  carry and two artifacts whose bytes differ may not publish one digest.
...
- The unit file names of one machine are one namespace, and its owner is the reading that derives
  them: `operator/read.nix` indexes every name it derives per machine, and two entries deriving one
  is `operator-entry-unit-file-collision`. The artifact name above carries the machine and cannot
  collide inside one, while the unit file name drops it, so neither row covers the other.
...
- An identity is nominal and name-deep. It carries no record shape, korora exposing a struct as a
  name, a verifier and an override, so two struct schemas may claim one `id` and the claim compares
  their names. The structural check is made where the value crosses the wire, against the type the
  consuming interface declares for the read it is delivered to: a value the far end publishes and
  the near end's own type refuses is `slot-read-type-mismatch`, so a shape is verified per value
...
- A mention of a value's path is asked one question about the machine it sits on: a site of a
  placed entry naming a generated value's path on a machine the value's delivery set does not
  contain is `vars-path-off-delivery-set`. An undeployed value is that rule where the set is empty,
  and it keeps `vars-not-deployed-opened` because the resolution differs: one asks for a declared
  read, the other for `deploy`.
...
- A file record's `owner`, `group` and `mode` are one value's and never one machine's: the same
  ...
  `root`, `0400`, and which of them enter the value's key is the key rule under Keys and identity.
- A configuration file's record carries the same three fields, `owner` and `group` defaulting to
  `root` and `mode` being required rather than defaulted. `configDataRecord` records the ownership
  on both of its branches, the incomplete one included: a reader must not be able to mistake an
  unfinished render for a record that does not say who reads the file.
