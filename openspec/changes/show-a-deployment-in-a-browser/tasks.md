...
- [ ] 1.5 Record which registration points this change does not trip, so a reader does not look for
...
- [ ] 2.1 Create the view's top-level directory with its own `flake-module.nix`, imported from
...
- [ ] 2.2 Add the directory to `classOf` in `tests/unit/layers.nix:54-85`, classed as the read-only
...
- [ ] 2.3 Add one `programs.mypy.directories` root for the view in `treefmt.nix:74-102`, with
...
- [ ] 2.4 Add the view's directory to `src` in `ruff.toml:4`, so a first-party import of the view's
...
- [ ] 3.1 Write the view's reading over one built deployment: it takes the target the operator's
  ...
  neither `nix` nor `ssh`.
- [ ] 3.2 Build the machines document off `manifest.json` and the plan: each machine, the entries
...
- [ ] 3.3 Build the values document off the manifest's `values` (`operator/read.nix:781-787`) and
...
- [ ] 3.4 Build the diagnostics document with all six fields of every row - identifier, subject,
...
- [ ] 4.1 Build the edge set out of the plan's resolved reads through the command's own reading:
...
- [ ] 4.2 Compute the layout as data: the column of an entry is the index of its strong component in
...
- [ ] 5.1 Render the page from those documents in the server: the machines and their entries as a
...
- [ ] 5.2 Spell every route the view answers so that its leading segment names no top-level entry of
...
- [ ] 6.1 Add the one live route: it calls the machine report's own status reading with the runner
  ...
  fabricated answers.
- [ ] 6.2 Ask only when a reader asks: no timer, no background poll, no answer served as current
...
- [ ] 7.1 Write the server over `http.server` from the standard library: the static routes answer
...
- [ ] 7.2 Bind the loopback interface, and refuse an instruction to listen anywhere else naming the
...
- [ ] 8.1 In the view's `flake-module.nix`, publish the server as `planner-view` in the namespaces
...
- [ ] 8.2 Publish the check that runs the view's tests under a name of its own rather than under the
...
- [ ] 8.3 Add the view to the one shell's packages (`devshells.nix:14-22`), so a reader following
  the document types the program's own name. Verify by entering the shell and running the view's
  `--help`.

## 9. The tests

- [ ] 9.1 Write the pytest layer beside the view's own modules, the way the command's
  ...
  machine.
- [ ] 9.2 Assert the page and not only the JSON: parse the served page with `html.parser` and assert
...
- [ ] 9.3 Add the two `consumer` suite cases the modified `tooling/consumer-surface` requirements
...
- [ ] 9.4 Add the two `layers` suite cases the modified `tooling/repository-shape` requirements
  ...
  .#checks.x86_64-linux.planner-tests`.
- [ ] 9.5 Record the one manual observation the suite cannot make: start the view against the worked
...
- [ ] 10.1 Write the view's own document under `docs/`: what it serves, the layout rule, that the
...
- [ ] 10.2 Add one row to the commands table of `README.md:86-93` naming the view, what it takes and
...
- [ ] 11.1 `nix eval --json '.#debug.failures'` returns `[]`; `nix build
...
- [ ] 11.2 In **one** edit, now that every scenario has its test: delete this change's three
