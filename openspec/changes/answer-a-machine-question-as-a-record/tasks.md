...
- [x] 1.1 Record no perf baseline, and record why: this change touches nothing `perf/eval.nix`
...
- [x] 2.1 In `cli/report.py`, add the frozen record one entry's answer is. Its fields are the ones
...
- [x] 2.2 In `cli/report.py`, add the frozen record one value's answer on one machine is: the value
...
- [x] 2.3 In `cli/remote.py`, delete `Holding.sentence` (`cli/remote.py:938-940`) from the record,
...
- [x] 3.1 In `cli/report.py`, make `_answered` (`:364-375`) return the record rather than a string:
...
- [x] 3.2 In `cli/report.py`, make `_read_status` (`:422-436`) and `_running` (`:471-486`) fill the
...
- [x] 3.3 In `cli/report.py`, make `_read_attachment` (`:489-521`) fill the held identity, the
...
- [x] 3.4 In `cli/report.py`, make `_absent` (`:266-282`) and `_seals` (`:298-337`) answer with the
...
- [x] 4.1 In `cli/report.py`, add the one function that renders a record into the line the command
...
- [x] 4.2 In `cli/report.py`, make `status` (`:139-168`) collect the records and hand each rendered
  ...
  the records are.
- [x] 4.3 In `cli/apply.py`, make `holding_lines` (`:205-215`) render through the function of 4.1
...
- [x] 5.1 Assert byte identity against the end-to-end folders that already match on those strings,
...
- [x] 5.2 Add the harness case that holds the contract in one place: render every record shape and
...
- [x] 6.1 In `cli/manifest.py`, give `Diagnostic` (`:123-131`) the `evidence` and `resolution`
...
- [x] 7.1 In `docs/operator.md`, state under "The command" that every line of a report is the
...
- [x] 7.2 In `docs/diagnostics.md`, state that a row reaches a program with all six fields and that
